...
module ${rv.name}_cpu_sub_system_ahb(
  //XOCC signals start
  dsa_clk,
...
//XOCC signals
% for ind in range(rv.num_xocc):
output wire [${rv.cmd_width[ind]}-1:0] dsa_cmd_buffer_${ind};
input wire [${rv.rsp_width[ind]}-1:0] dsa_rsp_buffer_${ind};
% endfor
...
${rv.name}_pa_cpu_top x_${rv.name}_pa_cpu_top (  
  ...
  .dsa_clk                              (dsa_clk             ),
% for ind in range(rv.num_xocc):
  .dsa_cmd_buffer_${ind}                (dsa_cmd_buffer_${ind}),
  .dsa_rsp_buffer_${ind}                (dsa_rsp_buffer_${ind}),
% endfor
  .dsa_rst                              (dsa_rst              ),
  .empty_cmd                            (empty_cmd            ),
  .full_rsp                             (full_rsp             ),
